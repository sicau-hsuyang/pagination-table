  ...
  <div class="base-table">
    ...
    >
      ...
      <base-column
        ...
        :prop="prop"
      ></base-column>
      <el-table-column v-if="showOperation" label="操作">
...
export default {
  ...
  created() {
    ...
    Object.entries(this.columns).forEach(([prop, column]) => {
      ...
      this.meta.serverSort && column.sortable && (column.sortable = "custom");
      column.prop = prop;
      ...
      if (this.name) {
        try {
          ...
          Object.prototype.toString.call(visibleConfig) != "[object Object]" &&
            (visibleConfig = {});
        } catch (ex) {
          visibleConfig = {};
        }
    ...
    loadData(queryParams, nextSort = true) {
      ...
      const fetchData =
        ...
        this.meta.fetchTableData(
          distParams,
          this.meta.serverSort && this.columns[sortParams[this.meta.orderField]]
            ? sortParams
            : {}
        );
      ...
      return list.map(item => {
        Object.entries(item).map(([prop, value]) => {
          this.columns[prop] &&
            ...
            (item[prop] = this.columns[prop].formatter(value, item));
        });
        return item;
      });
    },
